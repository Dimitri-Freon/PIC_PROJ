...
def convertCloudFromRosToOpen3d(ros_cloud):
    ...
    if "rgb" in field_names:
        ...
        #print("len data",len(cloud_data)) len data 70758
        
        # Get xyz
        xyz = [(x,y,z) for x,y,z,rgb in cloud_data ] # (why cannot put this line below rgb?)

        #rgb = [decode_rgb_from_ros(rgb) for x,y,z,rgb in cloud_data ]

        cloud_data_arr = np.array(cloud_data)
        #print(np.shape(cloud_data_arr)) (70758, 4)

        rgb_1 = cloud_data_arr[:,[3]]
        print(np.shape(rgb_1))
        rgb_attr = decode_rgb_from_ros(rgb_1)
        
        print("aaa",np.shape(rgb_attr))
        ...
        open3d_cloud.colors = open3d.utility.Vector3dVector(np.array(rgb_attr)/255.0)
